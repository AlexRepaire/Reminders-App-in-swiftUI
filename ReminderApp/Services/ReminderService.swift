...
class ReminderService {
    ...
    static func updateReminder(reminder: Reminder, editConfig: ReminderEditConfig) throws -> Bool {
        ...
        reminderToUpdate.reminderData = editConfig.hasDate ? editConfig.reminderDate : nil
    ...
    static func remindersByStatType(statType: ReminderStatsType) -> NSFetchRequest<Reminder> {
        let request = Reminder.fetchRequest()
        request.sortDescriptors = []
        ...
        case .today:
            ...
            request.predicate = NSPredicate(format: "(reminderData >= %@) AND (reminderData < %@)", today as NSDate, tomorrow! as NSDate)
        case .scheduled:
            request.predicate = NSPredicate(format: "(reminderData != nil OR reminderTime != nil) AND isCompleted = false")
        case .completed:
